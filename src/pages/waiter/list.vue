<template>
  <div id="waiter">
    ...
    <el-row :gutter="20" justify="end">
      ...
      <el-col :span="6">
        <div>
          <el-button type="primary" size="small" plain @click="addWaiter">添加</el-button>
          <el-button type="danger" size="small" plain @click.prevent="deleteWaiter()">批量删除</el-button>
        </div>
      </el-col>
      <!-- 搜索 -->
      <!-- {{name}} -->
    ...
    <div>
      ...
      >
        ...
        <el-table-column label="操作" align="center">
          ...
          <template v-slot:default="scope">
            <!-- 删除 -->
            <a href="" class="el-icon-delete" @click.prevent="deleteWaiter(scope.row.id)" />
            <span class="blank_margin" />
            <!-- 修改 -->
            <a href="" class="el-icon-edit-outline" @click.prevent="editWaiter(scope.row)" />
            <span class="blank_margin" />
            <!-- 详情 -->
    ...
    <!-- 对话框 -->
    <el-dialog title="添加/修改员工信息" :visible="visible" width="40%" @close="closeDialog">
      <el-form :model="form" :rules="rules" ref="waiterForm">
        ...
        <el-form-item label="手机" label-width="60px" prop="telephone">
         ...
         <el-form-item label="性别" label-width="60px" prop="gender">
...
export default {
  ...
  data() {
    return {
      title: '员工管理',
      typeTag: '', // 存放搜索类型realname和gender
      selectedDelete: [], // 存放批量删除的id的数组
      name: '', // 存放按照realname搜索的内容
      telephone: '', // 存放按照gender搜索的内容
      form: {},// 存放添加或修改的单个顾客信息
      rules:{//表单验证规则
        realname:[
  ...
  computed: {// 计算属性
    ...
    ...mapState('waiter', ['waiters', 'visible', 'params']),
    // 计算顾客信息总数，根据顾客状态过滤信息
  ...
  methods: {
    ...
    ...mapActions('waiter', ['loadWaiterData', 'deleteWaiterById', 'saveOrUpdateWaiter', 'batchDeleteWaiter']),
    // 显示模态框，关闭模态框,根据名字查询，跟号码查询,设置单条顾客信息
    ...mapMutations('waiter', ['showModal', 'closeModal', 'searchByName', 'searchBTel','SetWaiter']),
    ...
    loadData() {
      ...
      if (this.name) {
        ...
        this.searchByTel(this.telephone)
      }
      this.loadWaiterData()
    },
    ...
    deleteWaiter(id) {
      ...
      this.$confirm('此操作将永久删除信息,是否继续?', '提示', {
        ...
        if (id) {
          // 有id，单删
          // console.log("有",id);
          p = this.deleteWaiterById(id)// 映射action里的
        } else {
          // 无id,批量删除
          // console.log("无");
          p = this.batchDeleteWaiter(this.selectedDelete)// 映射action里的
          // console.log(p);
        }
        p.then((response) => {
